...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        #self.img_size = img_size # nb token == (real (PS-ks)/stride + 1)**2 , ks=ps=patch_size=vit patch size
    ...
    def forward(self, x):
        # x = b x emb x real PS x PS
        x = self.proj(x) 
        Hp, Wp = x.shape[2], x.shape[3]
        x = x.flatten(2).transpose(1, 2)  # b Ph*Pw c=emb / Ph*Pw=(real (PS-ks)/stride + 1)**2
...
def get_dsformer(config):
    if config["dataset_name"] == "ip":
        config["k"] = 4/5
    else:
        config["k"] = 2/5
    
   
    config.setdefault("ps", 2) # token size
    config.setdefault("kernel_size", 3)
    config.setdefault("use_pca", True)
    config.setdefault("n_comps", 30)
    
    model = DSFormer(dataset=config["dataset_name"], kernel_size=config["kernel_size"],  ps=config["ps"], k=config["k"], group_num=4, emb_dim=128)
...
# if __name__ == '__main__':
    ...
    # input = torch.rand(2,30,25,25).cuda()
