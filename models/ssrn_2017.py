...
class SSRN(nn.Module):
    def __init__(self, patch_size, band, classes):
        ...
        self.full_connection = nn.Sequential(
            nn.Dropout(p=0.5),
            nn.Linear(((patch_size-2)//5)**2 * 24, classes)
        )
...
def get_ssrn(config):
    ...
    model = SSRN(patch_size=config["patch_size"], band=config["n_bands"], classes=config["n_classes"])
    optimizer = torch.optim.RMSprop(model.parameters(), lr=config["lr"], weight_decay=1e-4)
